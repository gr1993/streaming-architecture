...
* 수집 단계(Netty) : 데이터 수집은 사용하는 프로토콜의 종류, 데이터 형식, 데이터 전달 방식(Pull 또는 Push 정책)에 따라 다양한 형태로 구현할 수 있다. 이번 실습에서는 원천 데이터인 Meetup 스트리밍 RSVP API 서버에서 청크(Chunk) 단위의 HTTP 또는 웹소켓을 통해 데이터를 제공하며, 이 중 웹소켓과 Netty 라이브러리를 활용하여 수집 단계를 구현한다. (HML 또한 구현하였으며 자세한 내용은 README 참고)
* 메시키 큐 단계(Kafka) : 카프카는 확장성과 고성능의 특징을 가지고 있으면서도 데이터 유실을 방지하는 옵션을 제공한다.
* 분석 단계(Apache Flink) : 책에서는 Storm을 사용하여 분석 단계를 구현하지만, 최근에는 Flink가 널리 사용되고 있어 분석 단계는 Flink로 구현하였다.
